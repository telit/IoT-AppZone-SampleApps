...
void Sms_Callback(M2MB_SMS_HANDLE h, M2MB_SMS_IND_E sms_event, UINT16 resp_size, void *resp_struct, void *myUserdata)
{
  ...
  switch(sms_event)
  {
    ...
    case M2MB_SMS_MEMORY_FULL_IND:
    {
      ...
      /*this event could be used to trigger a task and delete all stored
        SMS messages with m2mb_sms_get_storage_indexes*/
    ...
    case M2MB_SMS_GET_STORAGE_INDEXES_RESP:
    {
      ...
      res = m2mb_os_ev_set(sms_evHandle, EV_SMS_GET_STORAGE_IDXS, M2MB_OS_EV_SET);
      if(M2MB_OS_SUCCESS != res)
      {
        AZX_LOG_ERROR("Event bits setting failure: %d\r\n", res);
      }
    ...
    case M2MB_SMS_GET_STORAGE_STAT_RESP:
    {
      ...
      res = m2mb_os_ev_set(sms_evHandle, EV_SMS_GET_STORAGE_STAT, M2MB_OS_EV_SET);
      if(M2MB_OS_SUCCESS != res)
      {
        AZX_LOG_ERROR("Event bits setting failure: %d\r\n", res);
      }
    ...
    case M2MB_SMS_SET_TAG_RESP:
    {
      ...
      res = m2mb_os_ev_set(sms_evHandle, EV_SMS_SET_TAG, M2MB_OS_EV_SET);
      if(M2MB_OS_SUCCESS != res)
      {
        AZX_LOG_ERROR("Event bits setting failure: %d\r\n", res);
      }
...
INT32 sms_delTaskCB(INT32 type, INT32 param1, INT32 param2)
{
  (void)param1;
  (void)param2;
  ...
  M2MB_SMS_HANDLE smsHandle = (M2MB_SMS_HANDLE) type;
  int j;

  m2mb_os_ev_get(sms_evHandle, EV_SMS_MEM_FULL, M2MB_OS_EV_GET_ANY_AND_CLEAR, &curEvBits, M2MB_OS_WAIT_FOREVER);


  AZX_LOG_INFO("\r\nDEL, List unread messages...\r\n");
  retVal = m2mb_sms_get_storage_indexes(smsHandle, M2MB_SMS_TAG_MT_NOT_READ );
  if (retVal == M2MB_RESULT_SUCCESS)
  {
    AZX_LOG_TRACE("m2mb_sms_get_storage_indexes() - succeeded\r\n");
  }
  else
  {
    AZX_LOG_ERROR("m2mb_sms_get_storage_indexes() - failure\r\n");
  }

  if(M2MB_OS_SUCCESS ==  m2mb_os_ev_get(sms_evHandle, EV_SMS_GET_STORAGE_IDXS, M2MB_OS_EV_GET_ANY_AND_CLEAR, &curEvBits, M2MB_OS_MS2TICKS( 4000 )))
  {
    if (SMS_indexes_size != 0)
    {
      for(j=0; j< SMS_indexes_size; j++)
      {

        AZX_LOG_INFO("Delete pdu index %d\r\n", SMS_indexes[j]);
        retVal = m2mb_sms_delete(smsHandle, SMS_indexes[j]);
        if(M2MB_RESULT_SUCCESS != retVal)
        {
          AZX_LOG_ERROR("SMS delete operation request failed: %d\r\n", retVal);
        }
        else
        {
          if(M2MB_OS_SUCCESS !=  m2mb_os_ev_get(sms_evHandle, EV_SMS_DELETE, M2MB_OS_EV_GET_ANY_AND_CLEAR, &curEvBits, M2MB_OS_MS2TICKS( 4000 )))
          {
            AZX_LOG_ERROR("SMS delete operation failed\r\n");
          }
        }
      }
    }
  ...
  AZX_LOG_INFO("\r\nDEL, List read messages...\r\n");
  retVal = m2mb_sms_get_storage_indexes(smsHandle, M2MB_SMS_TAG_MT_READ );
  if (retVal == M2MB_RESULT_SUCCESS)
  {
    AZX_LOG_TRACE("m2mb_sms_get_storage_indexes() - succeeded\r\n");
  }
