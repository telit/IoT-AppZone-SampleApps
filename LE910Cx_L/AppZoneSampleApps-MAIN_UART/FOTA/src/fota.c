...
INT32 fotaTask(INT32 type, INT32 param1, INT32 param2)
{
	...
	case INITFOTA:
      ...
      readConfigFromFile(); /*try to read configuration from file (if present)*/
		...
		if(-1 == (fd = m2mb_fs_open(gSESSION_FILE, M2MB_O_RDONLY)))
		{
			...
			{
				AZX_LOG_ERROR("Unexpected filesystem error when checking for session file %s: %d\r\n", gSESSION_FILE, last_errno);
				return -1;
			}
		}
		else
	...
	case DOWNLOAD:
		//FTP
		fotaDwFtp.socketCfg.cid = gPDP_CTX;
		fotaDwFtp.socketCfg.cid_active = false;
		sprintf( fotaDwFtp.sessionCfg.URI, "%s/%s", gFTP_ADDR, gFILE_URI );
		sprintf( fotaDwFtp.sessionCfg.user, "%s", gFTP_USER );
		sprintf( fotaDwFtp.sessionCfg.psw, "%s", gFTP_PASS );
	...
	case UPDATE:
		...
		else
		{
			...
			if ( retVal == M2MB_RESULT_SUCCESS )
			{
				...
				if (-1 == m2mb_fs_open(gSESSION_FILE, M2MB_O_RDONLY | M2MB_O_CREAT, M2MB_ALLPERMS))
				{
...
INT32 mainTask(INT32 type, INT32 param1, INT32 param2)
{
	...
	case INIT:
		...
		retVal = m2mb_pdp_activate(pdpHandle, gPDP_CTX, (CHAR *) gAPN, (CHAR *) gAPN_UserName, (CHAR *) gAPN_Password, M2MB_PDP_IPV4);
		if ( retVal != M2MB_RESULT_SUCCESS )
		{
