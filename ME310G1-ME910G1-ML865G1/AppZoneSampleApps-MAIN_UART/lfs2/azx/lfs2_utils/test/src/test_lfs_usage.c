...
static TEST_RESULT_E testAllInOneFunction ( void )
{
	...
	if (azx_lfsWriteByContext("file002.txt",
									strlen(content),
									(UINT8*) content,
									AZX_LFSWRITE_MODE_CREAT,
									&lfs2,
	...
	if (azx_lfsWriteByContext("file001.txt",
							  strlen(content),
							  (UINT8*) content,
							  AZX_LFSWRITE_MODE_CREAT,
							  &lfs2,
	...
	if (azx_lfsWriteByContext("file003.txt",
									strlen(content),
									(UINT8*) content,
									AZX_LFSWRITE_MODE_CREAT,
									&lfs2,
	...
	if (azx_lfsWriteByContext("file004.txt",
									strlen(content),
									(UINT8*) content,
									AZX_LFSWRITE_MODE_CREAT,
									&lfs2,
	...
	memset(buffer, 0x00, sizeof(buffer));
	if (azx_lfsReadByContext("file004.txt",
			  	  	  	  	buffer,
							size,
							&lfs2) != LFS2_ERR_OK)
	{
