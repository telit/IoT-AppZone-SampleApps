...
UINT16 gMY_SERVER_LISTEN_PORT = 0;
...
int readConfigFromFile(void)
{
  ...
  char *_PDP_CDX = NULL;
  char *_MY_SERVER_LISTEN_PORT = NULL;
  ...
  if(fd != -1)
  {
    ...
    //MY_SERVER_LISTEN_PORT
    _MY_SERVER_LISTEN_PORT = mystrtok(&p, NULL,',');
    mystrtok(&p, NULL,'\n'); //strip description



    if( !_APN || !_APN_USERNAME || ! _APN_PASSWORD || !_PDP_CDX || !_MY_SERVER_LISTEN_PORT )
    {
    ...
    else
    {
      ...
      gMY_SERVER_LISTEN_PORT = atoi(_MY_SERVER_LISTEN_PORT);

      AZX_LOG_INFO("Set APN to: <<%s>>\r\n", gAPN);
      AZX_LOG_INFO("Set APN USER to: <<%s>>\r\n", gAPN_UserName);
      AZX_LOG_INFO("Set APN PASS to: <<%s>>\r\n", gAPN_Password);
      AZX_LOG_INFO("Set PDP_CDX to: %u\r\n", gPDP_CTX);
      AZX_LOG_INFO("Set MY_SERVER_LISTEN_PORT to: %u\r\n", gMY_SERVER_LISTEN_PORT);
      return 1;
    }
  }
  else
...
void configureParameters(void)
{
  ...
  gPDP_CTX = PDP_CTX;
  gMY_SERVER_LISTEN_PORT = MY_SERVER_LISTEN_PORT;
}
