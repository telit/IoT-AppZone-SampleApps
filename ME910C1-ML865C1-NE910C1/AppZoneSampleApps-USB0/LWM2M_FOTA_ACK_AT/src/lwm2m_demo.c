...
/*!
 * @brief        AT URC callback, called with #LWM2MFOTARING URCs arrive
 *
 *
 * @param[in]    msg     The incoming urc message
 *
 */
static void fotaring_urc_cb(const CHAR* msg);
...
static void fotaring_urc_cb(const CHAR* msg)
{
  ...
  AZX_LOG_TRACE("fotaring_urc_cb: %s\r\n", pURC);
  if(strstr(pURC, "DOWNLOAD"))
  {
    AZX_LOG_TRACE("\r\n\r\nFOTA DOWNLOAD urc!\r\n");
...
static INT32 atTaskCB(INT32 len, INT32 h, INT32 param2)
{
  ...
  if(rsp_len != -1)
  {
    ...
    char *p = strstr((const char*)g_at_rsp_buf,"#OTAEV");
    if(p)
    {
      char *e = strstr(p, "\r\n");
      if(e)
    ...
    {
      p = strstr((const char*)g_at_rsp_buf,"#LWM2MFOTARING");
      if(p)
      {
        char *e = strstr(p, "\r\n");
        if(e)
        ...
        AZX_LOG_TRACE("LWM2MFOTARING URC received.\r\n");
        fotaring_urc_cb(p);
        memset(g_at_rsp_buf,0, AT_BUF_SIZE);
        buf_index = 0;
      }
