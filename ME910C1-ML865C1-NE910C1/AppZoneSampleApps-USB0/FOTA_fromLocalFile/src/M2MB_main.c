...
static FOTA_RESULT_E startUpdate()
{
  ...
  if(retVal == M2MB_RESULT_SUCCESS)
  {
    ...
    writeFOTAUpgradeStatusFile(gFOTA_UP_INFO_PATH, fotaUPInfo.fotaFlag, currFWver);
    m2mb_fs_close(fd);
...
INT32 smartFotaTask(INT32 type, INT32 map, INT32 param2)
{
  ...
  if(copyFromFileSystemToDelta(gFOTA_WRITING_FILE_NAME) != FOTA_OK)
  {
...
void M2MB_main( int argc, char **argv )
{
  ...
  if (FOTAUpgradeStatusInit(gFOTA_UP_INFO_PATH) == M2MB_RESULT_FAIL)
  {
  ...
  else
  {
    azx_sleep_ms(500);
    AZX_LOG_INFO("\r\nRead stored data\r\n");
    readFOTAUpgradeStatusFile(gFOTA_UP_INFO_PATH);
