...
char gFOTA_STATUS_FILE[MAX_PARAMS_STR_LEN] = {0};
...
int readConfigFromFile(void)
{
  ...
  if(fd != -1)
  {
   ...
   //AUTH_TYPE
    ...
    //_DLTOBUF_FILE
    _FOTA_STATUS_FILE = mystrtok(&p, NULL,',');
    mystrtok(&p, NULL,'\n'); //strip description
    ...
    if( !_APN || !_APN_USERNAME || ! _APN_PASSWORD || !_PDP_CDX ||
            ...
            !_REMOTE_FOLDER || !_DLTOBUF_FILE|| !_FOTA_STATUS_FILE)
    {
    ...
    else
    {
      ...
      strcpy(tmp, _FOTA_STATUS_FILE);
      sprintf(gFOTA_STATUS_FILE, "%s%s",LOCALPATH,tmp);
      memset(tmp, 0, 100);
      ...
      AZX_LOG_INFO("Set FOTA_STATUS_FILE to: <<%s>>\r\n", gFOTA_STATUS_FILE);
      return 1;
    }
  }
  else
...
void configureParameters(void)
{
  ...
  strcpy(gFOTA_STATUS_FILE, FOTA_STATUS_FILE);
}
