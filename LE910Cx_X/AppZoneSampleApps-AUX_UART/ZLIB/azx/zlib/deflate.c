...
{
    return deflateInit2_(strm, level, Z_DEFLATED, MIN_WBITS, DEF_MEM_LEVEL,
    ...
    int stream_size)
{
    ...
    if (level != 0) level = 1;
#else
    if (level == Z_DEFAULT_COMPRESSION) level = Z_POOR_COMPRESSION;
#endif
    ...
    s->lit_bufsize = 1 << (memLevel + 3); /* 16K elements by default */
    ...
    int length;
{
    ...
    if (zmemcmp(s->window + match,
    	...
    	AZX_LOG_ERROR(" start %u, match %u, length %d\n",
                start, match, length);
        do {
        	AZX_LOG_ERROR( "%c%c", s->window[match++], s->window[start++]);
        } while (--length != 0);
        z_error("invalid match");
    }
    if (z_verbose > 1) {
    	AZX_LOG_ERROR("\\[%d,%d]", start-match, length);
